...
@section('stylesheets')
    <link rel="stylesheet" type="text/css" href="{{ asset('css/Backstage/curriculum/re_courseflow.css') }}">
@endsection
@section('content')
    <div style="border:1px solid #ccc" class="form_container container mb-5">
        @foreach ($courseflows as $courseflow)
            <form action="{{ route('re_courseflow', ['id' => $courseflow->id]) }}" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="row">
                    <div class="col-12">
                        <h1>修改課程流程圖</h1>
                            ...
                            <select name="year">
                                ...
                                @foreach ($courseframe_years as $courseframe_year)
                                    <option value="{{ $courseframe_year->year }}"
                                        {{ $courseflow->year == $courseframe_year->year ? 'selected' : '' }}>
                        ...
                        <div class="col-12 p-0 mb-2">
                            <label for="file" class="d-block"><b>{{ $courseflow->file }}</b></label>
