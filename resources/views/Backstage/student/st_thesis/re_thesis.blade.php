...
@section('stylesheets')
    <link rel="stylesheet" type="text/css" href="{{ asset('css/Backstage/student/st_thesis/re_thesis.css') }}">
@endsection
@section('content')
    <div style="border:1px solid #ccc" class="form_container container mb-5">
        @foreach ($thesis_ids as $thesis_id)
            <form action="{{ route('re_thesis', ['id' => $thesis_id->id]) }}" method="POST">
                @csrf
                <div class="row">
                    <div class="col-12">
                        <h1>新增碩士論文</h1>
                        ...
                        <select name="year">
                            ...
                            @foreach ($practical_topics_years as $practical_topics_year)
                                <option
                                    value="{{ $practical_topics_year->year }}" {{ $practical_topics_year->year == $thesis_id->year ? 'selected' : '' }}>
                        ...
                        <input type="text" name="author" value="{{ $thesis_id->author }}">

                        <label for="author-eng" class="d-block mt-3"><b>作者(英文)</b></label>
                        <input type="text" name="author-eng" value="{{ $thesis_id->eauthor }}">

                        <label for="topic" class="d-block mt-3"><b>題目(中文)</b></label>
                        <input type="text" name="topic" value="{{ $thesis_id->topic }}">

                        <label for="topic-eng" class="d-block mt-3"><b>題目(英文)</b></label>
                        <input type="text" name="topic-eng" value="{{ $thesis_id->etopic }}">
                        ...
                        <select name="teacher">
                            ...
                            @foreach ($teachers as $teacher)
                                <option value="{{ $teacher->name . '@' . $teacher->ename }}"
                                    {{ $teacher->name == $thesis_id->teacher ? 'selected' : '' }}>
                        ...
                        <div class="clearfix  mt-3">
                            ...
                            <a href="{{ route('back_thesis',['year' => $thesis_id->year]) }}"><button type="button"
