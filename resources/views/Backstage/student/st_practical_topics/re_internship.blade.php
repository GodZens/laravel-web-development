...
@section('stylesheets')
    <link rel="stylesheet" type="text/css" href="{{ asset('css/Backstage/student/st_practical_topics/re_internship.css') }}">
@endsection
@section('content')
    <div style="border:1px solid #ccc" class="form_container container mb-5">
        @foreach ($specialtopicpractices as $specialtopicpractice)
            <div class="row">
                <div class="col-12">
                    <h1><b>新增實習單位</b></h1>
                    <!-- <p>Please fill in this form to create an account.</p> -->
                    <hr>
                    <form action='{{ route('re_internship', ['id' => $specialtopicpractice->id]) }}' method='POST' class="col-12">
                        @csrf
                            ...
                            <select name="year" id="year" class="">
                                @foreach ($practical_topics_years as $practical_topics_year)
                                    <option value="{{ $practical_topics_year->year }}"
                                        {{ $specialtopicpractice->year == $practical_topics_year->year ? 'selected' : '' }}>
                        ...
                        <div class="col-8 mr-auto ml-auto mt-3">
                            ...
                            <input type="text" name="Internships" value="{{ $specialtopicpractice->unit }}" required>

                        </div>
                        <div class="col-8 mr-auto ml-auto mt-3">
                            <label for="Internships-eng"><b>實習單位(英文)</b></label>
                            <input type="text" name="Internships-eng" value="{{ $specialtopicpractice->eunit }}"
                                required>

                        </div>
                        <div class="col-8 mr-auto ml-auto mt-3">
                            <label for="contact-person"><b>聯絡人</b></label>
                            <input type="text" name="contact-person" value="{{ $specialtopicpractice->people }}"
                                required>
                        </div>
                        <div class="col-8 mr-auto ml-auto mt-3">
                            <label for="contact-phone"><b>連絡電話</b></label>
                            <input type="text" name="contact-phone" value="{{ $specialtopicpractice->number }}" required>
                        </div>
                        <div class="col-8 m-auto ">
                            ...
                            <select name="contact-mode" id="contact-mode" class="">
                                @foreach ($modes as $mode)
                                    <option value="{{ $mode->mode }}"
                                        {{ $specialtopicpractice->mode == $mode->mode ? 'selected' : '' }}>
                                        {{ $mode->mode }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-8 mr-auto ml-auto mt-3 d-flex align-items-cneter">
                            ...
                            <textarea name="editor" id="editor">{{ $specialtopicpractice->content }}</textarea>
                        </div>
                        <div class="col-8 mr-auto ml-auto mt-3 d-flex align-items-cneter">
                            <label for="contentaa-eng" class="mt-auto mb-auto"><b>實習內容(英文)</b></label>
                            <textarea name="editor_en" id="editor_en">{{ $specialtopicpractice->content_en }}</textarea>
                        </div>
                        <div class="clearfix">
